# Test Environment Variables for Cafe Brain Azure Resources (Simplified)
...
Write-Host ""
Write-Host "🔧 Testing Function App Environment Variables..." -ForegroundColor Cyan

try {
    $functionSettings = az functionapp config appsettings list --name $FunctionAppName --resource-group $ResourceGroupName --output json | ConvertFrom-Json
    ...
    if ($missingVars.Count -gt 0) {
        Write-Host ""
        Write-Host "⚠️ Missing variables in Function App: $($missingVars -join ', ')" -ForegroundColor Yellow
    } else {
        Write-Host ""
        Write-Host "✅ All environment variables found in Function App!" -ForegroundColor Green
    }
...
Write-Host ""
Write-Host "🌐 Testing Web App Environment Variables..." -ForegroundColor Cyan

try {
    $webSettings = az webapp config appsettings list --name $WebAppName --resource-group $ResourceGroupName --output json | ConvertFrom-Json
    ...
    if ($missingVars.Count -gt 0) {
        Write-Host ""
        Write-Host "⚠️ Missing variables in Web App: $($missingVars -join ', ')" -ForegroundColor Yellow
    } else {
        Write-Host ""
        Write-Host "✅ All environment variables found in Web App!" -ForegroundColor Green
    }
...
Write-Host ""
Write-Host "🔍 Testing Resource Availability..." -ForegroundColor Cyan
...
# Test Web App
Write-Host "Checking Web App status..."
try {
    $webStatus = az webapp show --name $WebAppName --resource-group $ResourceGroupName --query "state" --output tsv
...
Write-Host "==============" -ForegroundColor Yellow
Write-Host "1. If any variables are missing, run: .\azure-deployment\set-env-vars-simple.ps1" -ForegroundColor White
